<template>
  <div class="main" @dragenter.prevent @dragover.prevent @drop.prevent="onDrop"
    ...
    <div v-if="loading" style="margin: 20px 0; text-align: center">
      <span style="font-size: 20px; color:rgb(142,142,144);">加载中...</span>
    </div>
    <div v-else-if="!filteredFiles.length && !filteredFolders.length" style="margin: 20px 0; text-align: center">
      <span style="font-size: 20px; color:rgb(142,142,144);">没有文件</span>
    </div>
    <Dialog v-model="showContextMenu">
      <div
        style="height: 50px;display: flex; justify-content: center; align-items: center; padding:10px; background: rgb(119, 119, 119); margin: 0 0 10px 0; border-radius: 8px;">
...
export default {
  data: () => ({
    ...
    uploadProgress: null,
    uploadQueue: [],
    backgroundImageUrl: "/assets/bg-dark.webp"
  }),
    ...
    cwd: {
      handler() {
        ...
        document.title = this.cwd.replace(/.*\/(?!$)|\//g, "") === "/"
          ? "R2-CloudDrive"
          : `${this.cwd.replace(/.*\/(?!$)|\//g, "") || "/"} - R2-CloudDrive`;
      },
      immediate: true,
    },
  },
